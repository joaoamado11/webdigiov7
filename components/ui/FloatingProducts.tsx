...
function AnimatedProduct({ service, isKey, containerSize, paused, onReachCenter, onComplete, large }: {
  ...
  useEffect(() => {
    ...
    // Reset on every (re)run so that a containerSize/resize-driven re-render
    // restarts the loop instead of bailing on a stale cancelled flag. Without
    // this the cards freeze the first time containerSize updates after mount.
...
// ── Floating Products — self-contained animated showcase ──

export default function FloatingProducts({ height = 500 }: { height?: number }) {
